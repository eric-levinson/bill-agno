...
async def run_team(message: str) -> None:
    ...
    tools=[
        GridironTools(include_tools=["get_player_info_tool"])
    ],
    show_tool_calls=True,
    markdown=True,
    debug_mode=False,
    ...
    #await analytics_agent.aprint_response(message, stream=True)
    resp = await gridiron_team.arun(message)   # use async team API
    print(resp.content)
